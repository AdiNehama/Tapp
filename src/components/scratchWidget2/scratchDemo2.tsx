...
export function ScratchCard2({
  ...
  useEffect(() => {
    const initializeCanvas = () => {
      ...
      scratchImage.src = "/images/scratch-image2.webp";
      scratchImage.onload = () => {
        if (ctx && canvas) {
  ...
  const scratch = (point: Point) => {
    ...
    scratchPercentage.current = (transparentPixels / (pixels.length/3.5 )) * 100;

    if (scratchPercentage.current > 90 && !isScratched) {
      setIsScratched(true);
      setShowCollect(true);
  ...
  const generateZigzagPath = (
    ...
    const offsetY = canvasHeight * 0.4; // מוריד את המסלול כולו ב-20% מגובה הכרטיס
  ...
  const startAutoScratch = () => {
    if (!overlayRef.current || isRevealed || isAutoScratchingRef.current)
      return;
    ...
    const scratchNextPoint = () => {
      if (
        !isAutoScratchingRef.current ||
        currentPointIndex >= pathPoints.length ||
        isRevealed ||
        scratchPercentage.current > 80
      ) {
  ...
  return (
    <div className="scratch-card-yahtzee">
      <div className="scratch-background-yahtzee" />
      <SparklesBackground />

      <div className="scratch-content-yahtzee">
        <span className="prize-amount-yahtzee">
          <img src="/images/prize-yahtzee.webp" alt="Prize" />
        </span>
      </div>

      {!isRevealed && <canvas ref={overlayRef} className="scratch-overlay-yahtzee" />}

      {!hasStartedScratching && !isScratched && !isRevealed && (
        <div>
          <div
            className="scratch-text-yahtzee"
            onClick={handleAutoScratch}
            style={{ cursor: "pointer" }}
          >
            <img src="/images/scratch-button2.webp" alt="Scratch here" />
          </div>
          {showScratchPointer && (
      ...
      {showCollect && (
        <>
          ...
          <div className="collect-container-yahtzee">
            <div
              className="collect-button-wrapper-yahtzee"
